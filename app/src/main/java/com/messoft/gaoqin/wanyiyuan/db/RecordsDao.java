...
public class RecordsDao {
    ...
    public static RecordsDao getIntance(Context context) {
        if (recordsDao == null) {
    ...
    //删除单条数据
    public void deleteOneRecords(String record) {
        if (isHasRecord(record)) {
            recordsDb = recordHelper.getWritableDatabase();
//            recordsDb.execSQL("delete from records where name=" + record);
            recordsDb.execSQL("delete from records where name=?", new Object[]{record});
            recordsDb.close();
        }
    }
