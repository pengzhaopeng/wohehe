...
import com.messoft.gaoqin.wanyiyuan.adapter.CaigouAdapter;
 ...
 * 采购list
 */
public class CaigouFragment extends BaseFragment<CommonListBinding> {
    // 初始化完成后加载数据
    ...
    public static CaigouFragment newInstance(int type) {
        CaigouFragment fragment = new CaigouFragment();
...
//        showContentView();
        initRxBus();
        mAdapter = new CaigouAdapter(getBaseActivity());
    ...
    protected void initListener() {
        ...
        mAdapter.setOnItemClickListener(new OnItemClickListener<BeanOrderInfoList>() {
            ...
            public void onClick(BeanOrderInfoList beanOrderInfoList, int position) {
                CaigouInfoActivity.goPage(getActivity(), mType, beanOrderInfoList.getId());
            }
        });
    }
    ...
    private void requestData() {
        mGoldModel.getMyPurchaserBeanOrderList(getBaseActivity(),
                null,
                mType + "",
                null, null, null,
    ...
    private void initRxBus() {
        ...
        mRxBus.registerRxBus(mRxBus, RxBusMessage.class, rxBusMessage -> {
            //根据事件类型进行处理
            if (null != rxBusMessage && rxBusMessage.getType() == RxCodeConstants.REFRESH_MY_CG_ORDER) {
